// SPDX-FileCopyrightText: © 2025 Olivier Meunier <olivier@neokraft.net>
//
// SPDX-License-Identifier: AGPL-3.0-only

package configs

import (
	"crypto/rand"
	"crypto/subtle"
	"encoding/json"
	"errors"

	"github.com/google/uuid"
	"golang.org/x/crypto/blake2b"
	"golang.org/x/crypto/chacha20poly1305"

	"codeberg.org/readeck/readeck/pkg/base58"
)

const (
	signMACSize = 19 // Token hash size (152bit)
	nonceSize   = chacha20poly1305.NonceSizeX
	overhead    = chacha20poly1305.Overhead
)

var (
	// ErrInvalidSize is returned when an encoded string is not 128-bit long.
	ErrInvalidSize = errors.New("invalid encoded string size")
	// ErrInvalidData is returned when input data does not match its signature.
	ErrInvalidData = errors.New("invalid data")
)

type (
	// EncodingKey is a key that can be used to encode and decode
	// a variable that support JSON marshaling.
	EncodingKey []byte

	// SigningKey is a key that can be used to sign and verify
	// a base58 payload.
	SigningKey []byte
)

// Encode encodes the input data with chacha20-poly1305 using
// the key.
func (k EncodingKey) Encode(data []byte) ([]byte, error) {
	nonce := make([]byte, nonceSize, len(data)+overhead+nonceSize)
	rand.Read(nonce)

	aead, err := chacha20poly1305.NewX(k)
	if err != nil {
		return nil, err
	}

	return aead.Seal(nonce, nonce, data, nil), nil
}

// Decode decodes the input data with chacha20-poly1305 using
// the key.
func (k EncodingKey) Decode(encoded []byte) ([]byte, error) {
	if len(encoded) < nonceSize+overhead {
		return nil, ErrInvalidSize
	}

	nonce, ciphertext := encoded[:nonceSize], encoded[nonceSize:]
	aead, err := chacha20poly1305.NewX(k)
	if err != nil {
		return nil, err
	}

	content, err := aead.Open(nil, nonce, ciphertext, nil)
	if err != nil {
		return nil, errors.Join(ErrInvalidData, err)
	}

	return content, nil
}

// EncodeJSON marshals a variable to JSON and encodes it
// using the key.
func (k EncodingKey) EncodeJSON(v any) ([]byte, error) {
	data, err := json.Marshal(v)
	if err != nil {
		return nil, err
	}
	return k.Encode(data)
}

// DecodeJSON decodes an encoded value and unmarshals it into
// the variable v.
func (k EncodingKey) DecodeJSON(encoded []byte, v any) error {
	data, err := k.Decode(encoded)
	if err != nil {
		return err
	}

	return json.Unmarshal(data, v)
}

// Encode returns a base58 encoded token and its mac.
// The whole token is 280-bit long.
func (s SigningKey) Encode(raw string) (string, error) {
	id, err := base58.DecodeUUID(raw)
	if err != nil {
		return "", err
	}

	h, err := blake2b.New(signMACSize, s)
	if err != nil {
		return "", err
	}
	h.Write(id[:])

	res := make([]byte, len(id)+signMACSize)
	copy(res, id[:])
	copy(res[len(id):], h.Sum(nil))

	return base58.EncodeToString(res), nil
}

// Decode returns a token ID, using the signed, base58 encoded
// token. It checks its length and mac.
func (s SigningKey) Decode(encoded string) (string, error) {
	msg, err := base58.DecodeString(encoded)
	if err != nil {
		return "", err
	}
	if len(msg) != 16+signMACSize {
		return "", ErrInvalidSize
	}

	id, mac := msg[:16], msg[16:]

	h, err := blake2b.New(signMACSize, s)
	if err != nil {
		return "", err
	}
	h.Write(id)

	if subtle.ConstantTimeCompare(mac, h.Sum(nil)) != 1 {
		return "", ErrInvalidData
	}

	return base58.EncodeUUID(uuid.UUID(id)), nil
}
